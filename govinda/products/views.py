...
def update_cart(request,value1=""):
			perror=""
			psuccess=""
			qs=""
			gst=0
			...
			if request.POST or value1:
			 		if ('i_code' in request.POST and request.POST.get('i_code') != "") or value1:
			 				if 'order' in request.POST or value1:
			 					if value1:
			 						cart_list=[value1]
			 						order_list=['1']
			 					else:
...
def import_items(request):
				...
				Templates={
				'ITEMS':['ITEM_CODE','ITEM_NAME','UPD','DAMAGE','UNIT','QTY','PRICE','EXP_DATE','IGST','CGST','SGST','BU','ITEM_GROUP'],
																				...
																				if	not	rfile1.name.endswith('xlsx'):
																							...
																							if	'ITEMS'	not	in	wb.sheetnames:
																										...
																										if	flag==0:
																														...
																														grp_list=[]
																														grp_list=InventoryGroups.objects.values_list('name',flat=True)
																														mxrows = (ws.max_row)-1
																														count=0
																																		...
																																		if	row[0].value	is	not	None:
																																						...
																																						if	str(row_data[0])	!=	"":
																																										...
																																										if row_data[7].strip()=="":
																																											...
																																											exp_date=row_data[7].strip()
																																										
																																										if row_data[3].strip()=='Y':
																																											damage=True
																																										...
																																										try:
																																																...
																																																if row_data[2]=="Y":
																																																	...
																																																	obj.damage=damage
																																																	obj.unit=row_data[4]
																																																	obj.available_quantity=row_data[5]
																																																	obj.price=row_data[6]
																																																	obj.exp_date=exp_date
																																																	obj.igst=row_data[8]
																																																	obj.cgst=row_data[9]
																																																	obj.sgst=row_data[10]
																																																	obj.bu=BusinessUnit(name=row_data[11].strip(),desc=row_data[11].strip(),date=today)
																																																	obj.group=InventoryGroups(name=row_data[12].strip(),desc=row_data[12].strip(),date=today,bu=BusinessUnit(name=row_data[11].strip(),desc=row_data[11].strip(),date=today))
																																																	obj.save()
																																																	updated.append({'Item':row_data[0],'InventoryGroup':row_data[12].strip(),'BusinessUnit':row_data[11].strip()})


																																										except	InventoryItems.DoesNotExist:
																																																	bu_val=str(row_data[11]).strip()
																																																	ig_val=str(row_data[12]).strip()
																																																	#import pdb; pdb.set_trace()
																																																	if	bu_val	==	""	or	bu_val	not	in	bu_list:
																																																				buexclusions.append({'Item':row_data[0],'BusinessUnit':bu_val})
																																																					...
																																																					igexclusions.append({'Item':row_data[0],'InventoryGroup':ig_val})
																																																	else:
																																																		#import pdb; pdb.set_trace()
																																																		#today=datetime.date.today()
																																																		obj=InventoryItems(
																																																		...
																																																		damage=damage,
																																																		unit=row_data[4].strip(),
																																																		available_quantity=row_data[5].strip(),
																																																		price=row_data[6].strip(),
																																																		exp_date=exp_date,
																																																		igst=row_data[8].strip(),
																																																		cgst=row_data[9].strip(),
																																																		sgst=row_data[10].strip(),
																																																		bu=BusinessUnit(name=row_data[11].strip(),desc=row_data[11].strip(),date=today),
																																																		group=InventoryGroups(name=row_data[12].strip(),desc=row_data[12].strip(),date=today,bu=BusinessUnit(name=row_data[11].strip(),desc=row_data[11].strip(),date=today)),
																																																		date=today

																																																		)
																																																		obj.save()
																																																		inclusions.append({'Item':row_data[0],'InventoryGroup':row_data[12].strip(),'BusinessUnit':row_data[11].strip()})
																																		#count+=1
