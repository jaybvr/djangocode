									...
									if(int(order_list[i])>0):
										...
										try:
											...
											c_item=Cart.objects.get(pk=str(cart_list[i]).strip())
												...
												c_item=Cart(item_code=o_item.item_code,name=o_item.name,unit=o_item.unit,order_quantity=0,item_price=o_item.price,cgst=o_item.cgst,sgst=o_item.sgst,tax=0,item_net_price=0,order_price=0,bu=str(o_item.bu.name),item_group=str(o_item.group.name))
										c_item.order_quantity+=int(order_list[i])
										if c_item.cgst > 0 or c_item.sgst > 0:
											c_item.tax=round(((c_item.cgst+c_item.sgst)*c_item.item_price)/100,2)
										else:
											c_item.tax=0
										...
										if	(str(request.POST['item_code']).strip()) and (str(request.POST['bu']).strip()!="-----") and (str(request.POST['item_group']).strip()!=""):
												try:
													...
													error="Inventory Item with the same Item code  already present"
															...
															item=InventoryItems(item_code=item_code, name=name, unit=unit, available_quantity=item_quantity,price=item_price,cgst=cgst,sgst=sgst,date=date,bu=bu,group=group)
															item.save()
			...
			if total_paid==order_price:
				try:
					cust= Customers.objects.get(pk=str(request.POST['mobile']).strip())
							...
							cust.save()
				order=Orders(order_id=rdm,order_status='Success',order_date=timezone.now(),order_total=order_price,item_price=iprice,tax=gst,cash=cash,online=online,transaction_id=trid,customer_mobile=Customers.objects.get(pk=cust.customer_mobile))
				order.save()
				for item in Cart.objects.all():
					ist=ItemSales(item_code=item.item_code,item_name=item.name,unit=item.unit,bu=item.bu,group=item.item_group,order_quantity=item.order_quantity,item_price=item.item_price,cgst=item.cgst,sgst=item.sgst,tax=item.tax,item_net_price=item.item_net_price,order_price=item.order_price,customer_mobile=Customers.objects.get(pk=cust.customer_mobile),order_id=Orders.objects.get(pk=order.order_id))
					ist.save()
					item.delete()
			...
			if o_select== 'Today':
				...
				if d_from and d_to:
					...
					if d_from > d_to:
						...
						qs=Orders.objects.filter(order_date__range=[d_from, d_to])
				else:
...
def gen_summary_repo(request):
		...
		if request.GET:
				...
				context = {'bu_dict':bu_dict,'total_tax':total_tax,'total_order':total_order,'total_cash':total_cash,'total_online':total_online}
				# Create a Django response object, and specify content_type as pdf
				response = HttpResponse(content_type='application/pdf')
				response['Content-Disposition'] = 'attachment; filename='+fname
				# find the template and render it.
		...
		if li:
			...
			if li_bu:
				for j in li_bu:
					...
					li_dict[j]=qs
		#import pdb; pdb.set_trace()
		return render(request,'products/ListAlerts.html',{'za_dict':za_dict,'li_dict':li_dict})
...
def get_inventory(request):
		...
		return render(request,'products/GetInventory.html')
...
def gen_alert_repo(request):
		...
		if request.GET:
				...
				context = {'za_dict':za_dict,'li_dict':li_dict}
				# Create a Django response object, and specify content_type as pdf
				response = HttpResponse(content_type='application/pdf')
				response['Content-Disposition'] = 'attachment; filename='+fname
				# find the template and render it.
